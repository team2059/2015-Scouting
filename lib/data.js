...
module.exports = function(data) {
    ...
    data.getTeamAverages = function(team, callback) {
        data.db.get("SELECT team as team, AVG(4 * (b6 * 6 + b5 * 5 + b4 * 4 + b3 * 3 + b2 * 2 + b1)) as binscore, AVG(2 * (t6 + t5 + t4 + t3 + t2 + t1)) as totescore, AVG(2 * (c1 + c2 + c3 + c4)) as coopscore, AVG(4 * a4 + 6 * a3 + 8 * a2 + 20 * a1) as autoscore, AVG(4 * (b6 * 6 + b5 * 5 + b4 * 4 + b3 * 3 + b2 * 2 + b1) + 2 * (t6 + t5 + t4 + t3 + t2 + t1) + 2 * (c1 + c2 + c3 + c4) + 4 * a4 + 6 * a3 + 8 * a2 + 20 * a1) as totalscore from match_scouting WHERE team = ? AND scouter IS NOT NULL", team, function(err, data) {
			if (err) {
			    return callback(err);
			}
			if (!data.team) {
